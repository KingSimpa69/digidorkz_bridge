...
const BridgeUI = ({isConnected,chainId,address,Provider,alert}) => {
    ...
    const checkApproval = async (id) => {
        try {
            ...
            approved === CONFIG[chain].bridgeContract ? setInfoText(`Bridge Digi Dork #${id} for ${FEE} ETH`) : 
            setInfoText(`Digi Dork #${id} needs approval`)
    ...
    const exists = async (id) => {
        ...
        try {
            ...
            if(isAddress(owner) && owner.toLowerCase(owner) === address.toLowerCase()){
                ...
                setInfoText("")
                setButtonText("You don't own this Digi Dork")
    ...
    const approve = async () => {
        ...
        try {
            ...
            await tx.wait()
            alert("success",`Digi Dork #${input} approved`)
        } catch (e) {
            checkApproval(input)
            //console.log(e)
        } finally {
